...
export class UtilService {
    ...
    getTimeMsec(timestr: string): number { // time in format HH:MM:SS
        const dat = new Date();
        dat.setHours(parseInt(timestr.substring(0, 2), 10),
            parseInt(timestr.substring(3, 5), 10),
            parseInt(timestr.substring(6, 8), 10));
        return ((dat.getTime() - (dat.getTimezoneOffset() * 60000)) % msDay);
